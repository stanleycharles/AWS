...
## How to design an Advanced Dynamic VPN-BGP connection between site-to-site infrastructure and AWS cloud
...
The AWS side ``on the left`` will be created with ``1`` cloud formation template. And the on-premises side ``on the right`` will use another.

This demonstration use AWS ``to simulate`` the on-premises environment, so we will be configuring ``2 Linux-Based VPN servers`` which use ``strongSwan`` to provide the ``IPsec`` connectivity and a system known as ``FRR`` to do the ``BGP``.
...
Now, We've got ``2 customer gateway objects`` created and these represent the routers within your ``simulated on-premises environment``.
...
Now we are going to create the ``AWS side of the VPN architecture``. We are going to create ``2 VPN attachements`` for the ``transit gateway`` which already exists within the AWS environment. We will be selecting the ``accelerated VPN option`` fot these VPN attachements. And this means that AWS will create ``Accelerated VPN Endpoints`` 2 per connections. These accelerated endpoints provide ``transit back`` to the transit gateway over the ``AWS global network``. The Process also creates VPN connections ``to connect`` the accelerated VPN endpoints to the on premises routers using ``IPsec tunnels``, and this is ``2`` per connections.
The tunnels ``won't be active`` at this moment because that will require additional configuration.
...
If we are scroll to the right, we will note that one of the connections relates to ``ONPREM-ROUTER1`` and the another one relates to ``ONPREM-ROUTER2``. And that's as intended because each of these connections will connect from 1 specific ``customer gateway`` into a ``AWS``.
...
Then we are going ``to establish`` the ``IPsec tunnels`` by configuring the ``on-premises`` side of the architecture and get started ``activating`` the ``4`` IPsec tunnels that we are going to be using. So that's ``2`` tunnels from each customer router, ``1`` to each of the AWS VPN endpoints. So this is how we archieve the ``High Availability``. We've got ``2 customer premises routers`` and ``2 VPN connections``, each using 2 VPN endpoints so this ensures that we've got ``High Availability`` at ``both`` the AWS side and the on-premises side. 

In this part, There's going to be a ``lot of command line work`` that we need to do to configure the ``on-premises routers`` to be able to establish ``IPsec tunnels`` between the ``on-premises`` side and the ``AWS`` side.
...
Now, at this point we are going ``to copy`` all of these ``3`` IPsec configuration files ``ipsec.conf``, ``ipsec.secrets``, ``ipsec-vti.sh`` into the ``etc`` directory and make the script file executable with a ``chmod``. Then Restart the ``ONPREMS-ROUTER1`` EC2.
...
In the ``AWS management console``, we are going to utilize the ``IPsec tunnels`` and configure ``BGP`` to work across those tunnels and exchange routing information with AWS.
...
So we are going ``to change`` that, we are going to configure ``BGP`` to run across ``all`` of these IPsec tunnels.

Go back to the ``ONPREMS-ROUTER1`` and select ``Session Manager``.
In order to configure FRR routing's BGP capability, we need to go into the shell that's used to configure that product. Follow these command line.
...
This ``simulation`` could be applied in ``Real World Implementations``. We have now implemented an ``Extreme Availability Implementation of Site-To-Site VPN`` using ``Global Accelerator``. It uses ``dynamic routing``, performance is increated by using the ``AWS Global Network`` and it has ``Extreme High Availability`` with ``2 customer locations`` ``2`` VPN connections and each of those has ``2`` tunnels going from ``2`` AWS endpoints back to the customer endpoints.
